...
import {
  ...
  DotsList,
  DotsListItem,
} from "./Assistants.styled";

const isMobile = process.browser.innerWidth < 768;

export const Assistants = ({ items }) => {
              ...
              <Inner>
                {item.node.featuredImage.node.mediaItemUrl && (
                  <img
                    ...
                    alt={
                      item.node.featuredImage.node.mediaItemUrl || "Assistente"
                    }
                  />
                )}
                <ListContent>
                  <h3>{item.node.title}</h3>
                  <ListParagraph>{parse(item.node.content)}</ListParagraph>
                  <Link
                    href={{
